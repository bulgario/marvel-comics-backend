...
class FavoriteController {
  async addFavoriteComic(req, res, next) {
    ...
    connection.query(query, [id_user, id_api_comic], (error, results) => {
      if (error) {
        return res.status(500).send({ error: error, message: 'Comic já foi favoritada' });
      }
  ...
  async addFavoriteCharacter(req, res, next) {
    ...
    connection.query(query, [id_user, id_api_comic], (error, results) => {
      if (error) {
        return res.status(500).send({ error: error, message: 'Character já foi favoritada' });
      }
